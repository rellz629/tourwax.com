...
/**
 * Ranked "Top 5" strip rendered above an index page's main grid. Uses an ordered
 * list so the 1-5 ranking is conveyed semantically; the visible numerals are
 * decorative (aria-hidden).
 */
export default function TopStrip({ title, items }: Props) {
  if (items.length === 0) return null;

  return (
    <section className="mb-12" aria-labelledby="top-strip-heading">
      <h2 id="top-strip-heading" className="text-2xl md:text-3xl font-black mb-5">
      ...
      <ol className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        {items.map((item, i) => (
          <li key={item.href} className="h-full">
            <Link
              href={item.href}
              className="group flex flex-col h-full bg-white rounded-xl shadow-md hover:shadow-2xl card-hover overflow-hidden border border-gray-100"
            >
