...
import { generateVenueMetadata, generateCanonicalUrl, pagedTitle, SITE_URL } from '@/lib/seo';
...
// Shared by app/venues/[slug]/page.tsx (page 1) and
// app/venues/[slug]/page/[page]/page.tsx (page 2+). Pagination lives in the
...
const getVenueEvents = cache(async function getVenueEvents(venueIds: string[]) {
  ...
  return dedupeEvents(venueEvents, (row) => ({
    name: row.event.name,
...
export async function venueStaticParams() {
  const now = new Date();
...
export async function venueMetadata(venueSlug: string, currentPage: number): Promise<Metadata> {
  ...
  // equity still flows to the event and artist pages beneath them.
  if (currentPage > 1) {
    return {
      ...metadata,
      title: pagedTitle(metadata.title, currentPage),
      alternates: { canonical: generateCanonicalUrl(`/venues/${match.canonicalSlug}/page/${currentPage}`) },
...
export async function VenuePageView({ venueSlug, currentPage }: ViewProps) {
  const match = await getVenueBySlug(venueSlug);
  ...
  if (venueSlug !== match.canonicalSlug) {
    permanentRedirect(
      currentPage > 1
        ? `/venues/${match.canonicalSlug}/page/${currentPage}`
        : `/venues/${match.canonicalSlug}`
    );
  }
