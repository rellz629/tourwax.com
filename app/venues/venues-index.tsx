...
import { generateVenuesIndexMetadata, generateCanonicalUrl, SITE_URL } from '@/lib/seo';
...
//   /venues                 page 1, no filters   (app/venues/page.tsx)
//   /venues/page/[page]     pages 2+, no filters (app/venues/page/[page]/page.tsx)
//   /venues/browse?...      search/state, dynamic, noindex (app/venues/browse/page.tsx)
...
export async function venuesIndexMetadata(currentPage: number, filters: VenueFilters): Promise<Metadata> {
  ...
  // Deep pages of the unfiltered list are self-canonical, noindex, follow.
  if (currentPage > 1) {
    base.title = `Concert Venues ${new Date().getFullYear()} - Page ${currentPage}`;
    base.robots = { index: false, follow: true };
    if (!q && !state) {
      base.alternates = { ...base.alternates, canonical: generateCanonicalUrl(`/venues/page/${currentPage}`) };
    }
  }
  return base;
}

export async function VenuesIndexView({ currentPage, filters }: ViewProps) {
  const q = (filters.q || '').trim();
  const state = (filters.state || '').trim();
  const now = new Date();
  ...
  const basePath = filterQs ? `/venues/browse?${filterQs}` : '/venues';
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {/* Filters */}
        <form action="/venues/browse" method="GET" role="search" className="mb-10 flex flex-col sm:flex-row gap-2 max-w-2xl">
