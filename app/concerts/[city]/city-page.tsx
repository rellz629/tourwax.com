...
import { generateCityMetadata, generateCanonicalUrl, pagedTitle, SITE_URL } from '@/lib/seo';
...
// Shared by app/concerts/[city]/page.tsx (page 1) and
// app/concerts/[city]/page/[page]/page.tsx (page 2+). Pagination lives in the
...
export async function cityMetadata(citySlug: string, currentPage: number): Promise<Metadata> {
  ...
  // equity still flows to the event and venue pages beneath them.
  if (currentPage > 1) {
    return {
      ...metadata,
      title: pagedTitle(metadata.title, currentPage),
      alternates: { canonical: generateCanonicalUrl(`/concerts/${citySlug}/page/${currentPage}`) },
...
export async function CityPageView({ citySlug, currentPage }: ViewProps) {
  const cityInfo = await getCityInfo(citySlug);
