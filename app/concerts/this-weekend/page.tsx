...
import { generateThisWeekendMetadata, SITE_URL } from '@/lib/seo';
...
function getWeekendRange() {
  ...
  const start = day === 0 || day === 6 || day === 5 ? now : friday;
  const sunday = new Date(friday);
  sunday.setDate(friday.getDate() + (day === 0 ? 0 : (7 - friday.getDay()) % 7));
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return generateThisWeekendMetadata(weekendEvents.length);
}
...
export default async function ThisWeekendPage() {
  ...
  // Group by day
  const byDay = new Map<string, typeof weekendEvents>();
  for (const row of weekendEvents) {
  ...
  const breadcrumbSchema = generateBreadcrumbSchema([
    ...
    { name: 'This Weekend', url: `${SITE_URL}/concerts/this-weekend` },
  ]);

  const breadcrumbItems = [
    { name: 'Home', url: '/' },
    { name: 'Concerts', url: '/concerts' },
    { name: 'This Weekend', url: '/concerts/this-weekend' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-12">
          <h1 className="text-5xl md:text-6xl font-black mb-4">
            Concerts <span className="gradient-text">This Weekend</span>
          </h1>
          <p className="text-xl text-gray-600">
            {weekendEvents.length} show{weekendEvents.length === 1 ? '' : 's'} this weekend
          </p>
        ...
        {weekendEvents.length === 0 ? (
          <div className="bg-white rounded-xl shadow-md p-12 text-center border border-gray-100">
            <p className="text-gray-500 text-lg">No concerts this weekend. Check out <Link href="/concerts/this-week" className="text-orange-500 hover:text-orange-600 font-medium">this week&apos;s shows</Link>.</p>
          </div>
        ) : (
