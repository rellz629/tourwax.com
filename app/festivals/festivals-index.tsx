...
import { generateFestivalsIndexMetadata, generateCanonicalUrl, pagedTitle, SITE_URL } from '@/lib/seo';
...
// Shared by app/festivals/page.tsx (page 1) and
// app/festivals/page/[page]/page.tsx (page 2+). Pagination lives in the path,
// not searchParams, so both routes stay static/ISR.
interface ViewProps {
  currentPage: number;
}

export async function festivalsIndexMetadata(currentPage: number): Promise<Metadata> {
  const base = await generateFestivalsIndexMetadata();
  if (currentPage <= 1) return base;
  // Deep pages are navigational: self-canonical, noindex, follow.
  return {
    ...base,
    title: pagedTitle(base.title, currentPage),
    alternates: { ...base.alternates, canonical: generateCanonicalUrl(`/festivals/page/${currentPage}`) },
    robots: { index: false, follow: true },
  };
}

export async function FestivalsIndexView({ currentPage }: ViewProps) {
